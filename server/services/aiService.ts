...
export class AIService {
  public static parseQuery(rawQuery: string): ParsedIntent {
    ...
    if (geoType === 'national') {
      if (q.includes('up') || q.includes('uttar pradesh') || q.includes('उत्तर प्रदेश')) {
        ...
        stateCode = 'IN-MP';
      } else if (q.includes('maharashtra') || q.includes('महाराष्ट्र') || q.includes('mumbai') || q.includes('pune')) {
        geoType = 'state';
        geoName = 'Maharashtra';
        stateCode = 'IN-MH';
        ...
        geoName = 'Rajasthan';
        stateCode = 'IN-RJ';
      } else if (q.includes('karnataka') || q.includes('कर्नाटक') || q.includes('bangalore') || q.includes('bengaluru')) {
        geoType = 'state';
        geoName = 'Karnataka';
        stateCode = 'IN-KA';
    ...
    if (q.includes('forest') || q.includes('jungle') || q.includes('van') || q.includes('वन') || q.includes('ped')) {
      indicator = 'forest';
    } else if (q.includes('urban') || q.includes('built-up') || q.includes('builtup') || q.includes('shahar') || q.includes('city') || q.includes('construction')) {
      indicator = 'builtup';
    } else if (q.includes('water') || q.includes('paani') || q.includes('jal') || q.includes('lake') || q.includes('talab') || q.includes('जल')) {
      indicator = 'waterbodies';
    } else if (q.includes('barren') || q.includes('banjar') || q.includes('uncultivable') || q.includes('बंजर')) {
      indicator = 'barren';
    } else if (q.includes('irrigation') || q.includes('sinchai') || q.includes('irrigated') || q.includes('सिंचाई')) {
      indicator = 'irrigated';
  ...
  public static async answerQuery(queryText: string): Promise<AIQueryResponse> {
    ...
    if (metrics.direction === 'decreasing') {
    ...
    const response: AIQueryResponse = {
      ...
      sources: [
        ...
        { name: 'State Land Record Portal & Survey Reports', year: '2025', url: 'https://updes.up.nic.in', datasetId: 'DS-UP-DES' },
      ...
      calculationBreakdown: {
        ...
        stepExplanation: `Computed exact decadal delta across normalized records from the Directorate of Economics and Statistics. The annualized CAGR is ${metrics.cagr}%.`
      }
    };
