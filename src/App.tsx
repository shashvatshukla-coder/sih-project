  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex">
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-72'}`}>
        <TopHeader collapsed={sidebarCollapsed} onOpenMobile={() => setMobileDrawerOpen(true)} />
        ...
        <main className="flex-1 p-4 md:p-6 lg:p-8 max-w-7xl w-full mx-auto">
          ...
          {activePage === 'map' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-left">
                <h2 className="text-base font-bold text-slate-900 dark:text-white">Interactive Geospatial Map Explorer</h2>
                <p className="text-xs text-slate-500">Select layers, play 2005–2025 timeline, and click any state to inspect district breakdowns.</p>
              </div>
              <IndiaMapExplorer />
            </div>
          )}
          ...
          {activePage === 'decision-support' && <PolicyLab />}
        ...
        <footer className="mt-auto py-6 px-4 md:px-8 border-t border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 text-xs text-slate-500 text-left backdrop-blur-sm">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-slate-400 pl-1">Adhering to MoA&FW 9-fold land classification standard and GODL India Open Data Guidelines.</p>
            </div>
            <div className="flex flex-wrap items-center gap-4 text-[11px] text-slate-400">
              <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-semibold"><ShieldCheck className="w-3.5 h-3.5" />Verified Ground-Truth Intelligence</span>
              <span>•</span>
              <button onClick={() => setShowSplash(true)} className="hover:text-brand-600 dark:hover:text-brand-400 transition-colors underline cursor-pointer">Replay Intro Splash</button>
              <span>•</span>
...
export const App: React.FC = () => <AppProvider><AppContent /></AppProvider>;
export default App;
